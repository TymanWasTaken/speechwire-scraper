...
await new Command()
	...
	.option(
		"-o, --old-tournaments",
		"Whether or not to display old tournaments in the picker",
	)
	.option(
		...
		while (true) {
			...
			if (round.bye?.includes(args[0])) {
				...
				if (args[1]) {
					await sendNotification(args[1], {
						...
						message: `Round ${options.round} was posted, but ${
							round.bye.join(" and ")
						} got the bye`,
			...
			if (matching) {
				...
				console.log(
					colors.brightCyan(
						`Room: ${matching.room}\nTeam one: ${
							matching.teamOne.join(" and ")
						}\nTeam two: ${matching.teamTwo.join(" and ")}\nJudge(s): ${
							matching.judges.join(", ")
						}`,
					),
				);
				if (args[1]) {
					await sendNotification(args[1], {
						title: `Round ${options.round} posted`,
						message: `Room ${matching.room}: ${
							matching.teamOne.join(" and ")
						} vs. ${matching.teamTwo.join(" and ")} (${
							matching.judges.join(", ")
						} judging)`,
						click: roundInfo.schematicsUrl,
						priority: NotificationPriority.High,
				...
				console.log(
					colors.bold.brightRed(
						`Round ${options.round} was posted, but ${args[0]} was not matched`,
					),
				);
				if (args[1]) {
					await sendNotification(args[1], {
